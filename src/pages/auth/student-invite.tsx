import { useEffect, useState } from 'react'
import { useParams, useNavigate } from 'react-router-dom'
import { Dumbbell, CheckCircle2, XCircle } from 'lucide-react'
import { Button } from '@/components/ui/button'
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from '@/components/ui/card'
import { useSignInWithGoogle, useAcceptInvite, useAuthListener } from '@/services/auth'
import { useAuthStore } from '@/stores/auth-store'

export default function StudentInvitePage() {
  useAuthListener()
  const { token } = useParams<{ token: string }>()
  const navigate = useNavigate()
  const { user, role, isLoading } = useAuthStore()
  const signIn = useSignInWithGoogle()
  const acceptInvite = useAcceptInvite()
  const [inviteAccepted, setInviteAccepted] = useState(false)

  // If user is logged in and has a token, try to accept the invite
  useEffect(() => {
    if (user && token && !inviteAccepted && !acceptInvite.isPending) {
      acceptInvite.mutate(token, {
        onSuccess: () => {
          setInviteAccepted(true)
          setTimeout(() => navigate('/student/home', { replace: true }), 2000)
        },
      })
    }
  }, [user, token, inviteAccepted, acceptInvite, navigate])

  // If already a student, redirect
  useEffect(() => {
    if (!isLoading && role === 'student') {
      navigate('/student/home', { replace: true })
    }
  }, [role, isLoading, navigate])

  if (isLoading) {
    return (
      <div className="flex min-h-screen items-center justify-center">
        <div className="h-8 w-8 animate-spin rounded-full border-4 border-primary border-t-transparent" />
      </div>
    )
  }

  return (
    <div className="flex min-h-screen items-center justify-center bg-gradient-to-br from-primary/5 via-background to-primary/10 p-4">
      <Card className="w-full max-w-md">
        <CardHeader className="text-center">
          <div className="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-2xl bg-primary">
            <Dumbbell className="h-8 w-8 text-primary-foreground" />
          </div>
          <CardTitle className="text-2xl font-bold">Invitación FitFlow</CardTitle>
          <CardDescription>
            Tu entrenador te invitó a unirte a su plataforma
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          {inviteAccepted ? (
            <div className="flex flex-col items-center gap-3 py-4">
              <CheckCircle2 className="h-12 w-12 text-green-500" />
              <p className="text-center font-medium">¡Invitación aceptada!</p>
              <p className="text-center text-sm text-muted-foreground">
                Redirigiendo a tu portal...
              </p>
            </div>
          ) : acceptInvite.isError ? (
            <div className="flex flex-col items-center gap-3 py-4">
              <XCircle className="h-12 w-12 text-destructive" />
              <p className="text-center font-medium">Error con la invitación</p>
              <p className="text-center text-sm text-muted-foreground">
                El link de invitación es inválido o ya fue utilizado.
              </p>
            </div>
          ) : !user ? (
            <Button
              className="w-full gap-2 py-6 text-base"
              size="lg"
              onClick={() => signIn.mutate()}
              disabled={signIn.isPending}
            >
              <svg className="h-5 w-5" viewBox="0 0 24 24">
                <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 0 1-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z" fill="#4285F4" />
                <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853" />
                <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05" />
                <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335" />
              </svg>
              {signIn.isPending ? 'Conectando...' : 'Aceptar invitación con Google'}
            </Button>
          ) : (
            <div className="flex flex-col items-center gap-3 py-4">
              <div className="h-8 w-8 animate-spin rounded-full border-4 border-primary border-t-transparent" />
              <p className="text-sm text-muted-foreground">Aceptando invitación...</p>
            </div>
          )}
        </CardContent>
      </Card>
    </div>
  )
}
